...
impl ProviderAdapter for EventbriteAdapter {
            ...
            .filter_map(|org| {
                ...
                Some(ProviderAccount {
                    ...
                    display_name: org
                        .get("name")
                        .and_then(Value::as_str)
                        .unwrap_or("Eventbrite organization")
                        .to_owned(),
    ...
    async fn publish(
        ...
        let create_url = self
            .api_base
            .join(&format!("organizations/{organization_id}/events/"))?;
        let event_body = json!({
            "event": {
        ...
        let ticket_url = self
            .api_base
            .join(&format!("events/{event_id}/ticket_classes/"))?;
        let mut ticket = json!({
            "name": ticket_name,
        ...
        if !free {
            let amount = context
                ...
                .and_then(Value::as_u64)
                .ok_or_else(|| {
                    ProviderError::Configuration("paid tickets require cost_minor".into())
                })?;
            ticket["cost"] = Value::String(format!("{currency},{amount}"));
        }
        let ticket_receipt = checked_json(
            self.http
                .post(ticket_url)
                .bearer_auth(tokens.access_token.expose_secret())
                .json(&json!({ "ticket_class": ticket }))
                .send()
                .await?,
        )
        .await?;
        ...
        let publish_receipt = checked_json(
            self.http
                .post(publish_url)
                .bearer_auth(tokens.access_token.expose_secret())
                .send()
