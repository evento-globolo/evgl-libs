...
impl EventDraft {
    pub fn validate(&self) -> Result<(), DomainError> {
        ...
        if !(1..=200).contains(&title_len) {
            return Err(DomainError::InvalidEvent("title must contain 1..=200 characters"));
        }
        if self.summary.chars().count() > 280 {
            return Err(DomainError::InvalidEvent("summary cannot exceed 280 characters"));
        }
...
impl CrossPostRequest {
    pub fn validate(&self) -> Result<(), DomainError> {
        if self.targets.is_empty() {
            return Err(DomainError::InvalidCrossPost("at least one target is required"));
        }
...
mod tests {
    ...
    fn aliases_parse_to_meta_page() {
        assert_eq!("facebook".parse::<ProviderKind>().unwrap(), ProviderKind::MetaFacebookPage);
    }
    #[test]
    fn all_provider_names_are_stable() {
        let names: Vec<_> = ProviderKind::ALL.into_iter().map(ProviderKind::as_str).collect();
        assert_eq!(names, vec!["eventbrite", "meetup", "meta_facebook_page", "craigslist", "generic_webhook"]);
    }
}
